    ...
    <div class="container">
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <p class="card-text">
                    {{ __('List and manage caters here.') }}
                    ...
                    <table class="table mb-0">
                        ...
                        @forelse ($users as $cater)
                            <tr>
                                ...
                                {{-- <td>
                                    @if ($cater->image)
                                        <a href="{{ asset('storage/'.$cater->image) }}" rel="noopener noreferrer" target="_blank">
                                            <figure class="figure">
                                                <img height="150px" src="{{ asset('storage/'.$cater->image) }}" class="figure-img  rounded" alt="...">
                                ...
                                <td>{{ @$cater->description }}</td>
                                <td>{{ @$cater->cater->city }}</td>

                                <td>{{ $cater->created_at ? $cater->created_at->format('d-m-Y H:i:s'): '' }}</td>
                                <td>
                                    <a class="btn btn-outline-dark btn-sm" href="{{ route('admin.caters.view', $cater->id) }}">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a class="btn btn-info btn-sm" href="{{ route('admin.caters.update', $cater->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <a class="btn btn-danger btn-sm"
                                        wire:click="deleteId({{ $cater->id }})"
            ...
            <div class="card-body border-top d-flex justify-content-between">
                {{ __('Showing :from to :to of :total Food Categories.', ['from' => $users->firstItem() ?: 0, 'to' => $users->lastItem() ?: 0, 'total' => $users->total()]) }}
