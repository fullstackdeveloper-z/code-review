<div class="aiz-user-sidenav-wrap position-relative z-1 shadow-sm">
    <div class="aiz-user-sidenav rounded overflow-auto c-scrollbar-light pb-5 pb-xl-0">
        ...
        <div class="btn-group">
      <button type="button" class="mt-2 btn btn-sm btn-outline-light dropdown-toggle text-white" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        Edit Profile
      </button>
      <div class="dropdown-menu">
         <a class="dropdown-item" href="{{ route('compare') }}">{{translate('My Comparison')}}</a>
        <a class="dropdown-item" href="{{ route('wishlists.index') }}">{{translate('My Favourites')}}</a>
        <a class="dropdown-item" href="{{ route('cart') }}">{{translate('My Cart')}}</a>
        <hr>       
        <a class="dropdown-item" href="{{ route('profile') }}">{{translate('Manage Profile')}}</a>
        ...
        <div class="sidemnenu mb-3">
          ...
          <!--<h4 style="margin-left:10px" class="h6 fw-600">{{ translate('Vendor Menu')}}</h4>-->
                        ...
                        <ul class="aiz-side-nav-list level-2 mm-collapse">
                            ...
                            <li class="aiz-side-nav-item">
                                <a href="{{ route('cancel-request-list') }}" class="aiz-side-nav-link {{ areActiveRoutes(['cancel-request-list'])}} ">
                                    <span class="aiz-side-nav-text">  {{ translate('Request Cancelled Delivery') }}</span>
                                </a>
                            </li>
                            <li class="aiz-side-nav-item">
                                <a href="{{ route('total-earnings') }}" class="aiz-side-nav-link {{ areActiveRoutes(['today-collection'])}}">
                ...
                @else
                    ...
                    @if(get_setting('classified_product') == 1)
                        <li class="aiz-side-nav-item">
                            <a href="{{ route('customer_products.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['customer_products.index', 'customer_products.create', 'customer_products.edit'])}}">
                                <i class="la la-exchange-alt aiz-side-nav-icon"></i>
                    ...
                    @if(Auth::user()->user_type == 'seller')
                        ...
                        <a href="#" class="aiz-side-nav-link">
                            <i class="las la-shopping-cart aiz-side-nav-icon"></i>
                            <span class="aiz-side-nav-text">My Store</span>
                            <span class="aiz-side-nav-arrow"></span>
                        </a>
                        <!--Submenu-->
                        <ul class="aiz-side-nav-list level-2 mm-collapse">
            ...
            <div class="text-center">
                <div class="heading-4 strong-700 mb-4">
                    @php
                        ...
                        //}
                    @endphp
                    <small class="d-block fs-12 mb-2">Outstanding Earnings</small>
                    <span href="{{ route('withdraw_requests.index') }}" class="btn btn-primary fw-600 fs-18">{{ single_price(Auth::user()->seller->admin_to_pay) }}</span>
                </div>
        ...
        @else
         <hr>
         <h4 class="h5 fw-600 text-center">{{ translate('Wallet Balance') }}</h4>
         @if (get_setting('wallet_system') == 1)
         @php
              $date = date("Y-m-d");
              $days_ago_30 = date('Y-m-d', strtotime('-30 days', strtotime($date)));
         ...
         <div class="widget-balance pb-3 pt-1">
            <div class="text-center">
                <div class="heading-4 strong-700 mb-4">
                    ...
                    <span href="{{ route('wallet.index') }}" class="btn btn-primary fw-600 fs-18 {{ areActiveRoutes(['wallet.index'])}}">{{ single_price(Auth::user()->balance) }} </span>
                </div>
                <table class="table table-borderless">
                     @php
                            $spendTotal = \App\Models\Order::where('user_id', Auth::user()->id)->where("payment_status", 'paid')->sum('grand_total');
                        @endphp
                    <tr>
                        <td class="p-1" width="60%">
                        ...
                        <td class="p-1 fw-600" width="40%">
                            {{ single_price($spendTotal) }}
                        </td>
                    </tr>
                    <tr>
                       @php
                            $spendTotal2 = \App\Models\Order::where('user_id', Auth::user()->id)->where("payment_status", 'paid')->where('created_at', '>=', $days_ago_60)->where('created_at', '<=', $days_ago_30)->sum('grand_total');
                        @endphp
                        <td class="p-1" width="60%">
                            {{ translate('Last Month Spent')}}:
                        </td>
                        <td class="p-1 fw-600" width="40%">
                          {{ single_price($spendTotal2) }}
        ...
        @endif
            <div>
                <a href="{{ route('support_ticket.index') }}" class="btn btn-block btn-soft-primary rounded-0">
                    </i>{{translate('Support Ticket')}}
                            @if($support_ticket > 0)<span class="badge badge-inline badge-success">{{ $support_ticket }}</span> @endif
                </a>
            </div>
    </div>
