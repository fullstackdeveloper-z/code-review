        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group row" id="category">
                    ...
                    <div class="col-lg-9">
                        @php
                            $categories = \App\Models\Category::where('parent_id', 0)->where('digital', 1)->with('childrenCategories')->get();
                        @endphp

                            <input type="hidden" name="category_id" value="{{ $product->category_id }}" id="categoryId">
                            <select class="form-control aiz-selectpicker" size="6" name="parent_id" id="category_id" data-live-search="true" required>
                                @php
                                    $midCategories = [];
                                @endphp
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if(@$product->category->parentCategory->parentCategory->id == $category->id || @$product->category->parentCategory->id == $category->id || $product->category_id == $category->id) selected  @endif  data-child = '{{ json_encode($category->childrenCategories) }}'  >{{ $category->getTranslation('name') }}</option>

                                    @if(@$product->category->parentCategory->parentCategory->id == $category->id || @$product->category->parentCategory->id == $category->id)
                                        @php
                                            $midCategories = $category->childrenCategories;
                                        @endphp
                                    @endif
                                @endforeach
                            </select>
                            ...
                            <div class="row ">
                                <div class="col-sm-12 col-lg-6 pt-2">
                                    <select class="form-control " size="6" name="midCategoryId" id="midCategoryId" data-live-search="true" @if(count($midCategories)) required  @endif>
                                        @php
                                            $lastCategories = [];
                                        @endphp
                                        @forelse ($midCategories as $midCate)
                                            <option @if(@$product->category->parentCategory->id == $midCate->id || $product->category_id == $midCate->id) selected  @endif value="{{ $midCate->id }}">{{ $midCate->name }}</option>
                                            @if(@$product->category->parentCategory->id == $midCate->id)
                                                @php
                                                    $lastCategories = $midCate->categories;
                                                @endphp
                                            @endif
                                        @empty
                                        @endforelse
                                ...
                                <div class="col-sm-12 col-lg-6 pt-2">
                                    <select class="form-control " size="6" name="lastCategoryId" id="lastCategoryId" data-live-search="true" @if(count($lastCategories)) required  @endif>
                                        @forelse ($lastCategories as $lastCate)
                                            <option @if($product->category->id == $lastCate->id) selected  @endif value="{{ $lastCate->id }}">{{ $lastCate->name }}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                </div>
...
@endsection
@section('script')
<script src="{{ asset('public/assets/js/bootstrap-selectsplitter.js') }}"></script>
<script>
    $(function() {

// generate mid select
let selectOne = $("#category_id");

selectOne.on('change', function() {
    let selectedParent = selectOne.children('option:selected');
    let selectedParentOptionData = selectedParent.data('child')
    let midOptions = '';
    $("#categoryId").val( selectOne.val());
    if(selectedParentOptionData.length) {
        $("#midCategoryId").prop('required', true);
        selectedParentOptionData.forEach(element => {
            midOptions += '<option value="'+element.id+'" data-child=\''+JSON.stringify(element.categories)+'\' >'+element.name+'</option>';
        });
    } else {
        $("#midCategoryId").prop('required', false);
    }
    $("#midCategoryId").html(midOptions);
    $("#lastCategoryId").html('');
});


// generate mid select
let selectTwo = $("#midCategoryId");

selectTwo.on('change', function() {
    let selectedMid = selectTwo.children('option:selected');
    let selectedMidOptionData = selectedMid.data('child');
    let lastOptions = '';
    $("#categoryId").val(selectTwo.val());
    if(selectedMidOptionData.length) {
        $("#lastCategoryId").prop('required', true);
        selectedMidOptionData.forEach(element => {
            lastOptions += '<option value="'+element.id+'" data-child="'+JSON.stringify(element.categories)+'" >'+element.name+'</option>';
        });
    } else {
        $("#lastCategoryId").prop('required', false);
    }
    $("#lastCategoryId").html(lastOptions);
});

$("#lastCategoryId").change(function() {
    $("#categoryId").val( $("#lastCategoryId").val());
})
